    ...
    public sealed partial class MainPage : Page
    {
        ...
        private void AnimateEnemy(ContentControl enemy, double from, double to, string propertyToAnimate)
        {
            ...
            DoubleAnimation animation = new DoubleAnimation()
            {
                From = from,
                To = to,
                Duration = new Duration(TimeSpan.FromSeconds(random.Next(2, 10)))
            };
        ...
        private void playArea_PointerMoved(object sender, PointerRoutedEventArgs e)
        {
            if(humanCaptured)
            {
                ...
                if(((Math.Abs(relativePosition.X - Canvas.GetLeft(human))) > human.ActualWidth) || (Math.Abs(relativePosition.Y - Canvas.GetTop(human))) > human.ActualHeight)
                {
                    humanCaptured = false;
                    human.IsHitTestVisible = true;
                } else
